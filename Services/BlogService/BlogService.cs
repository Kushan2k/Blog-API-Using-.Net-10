...
public class BlogService(ApplicationDbContext _context, IHttpContextAccessor _httpContextAccessor, ILogger _logger) : IBlogService
{
    ...
    public Task<IActionResult> DeleteBlogAsync(int id, int v)
    {
        try
        {
            ...
            if (blog.AuthorId != v)
            {
                return Task.FromResult<IActionResult>(new UnauthorizedObjectResult(new { message = "User not authorized to delete this blog", status = System.Net.HttpStatusCode.Unauthorized }));
            }

            _context.Blogs.Remove(blog);
            _context.SaveChanges();

            return Task.FromResult<IActionResult>(new OkObjectResult(new { message = "Blog deleted successfully", status = System.Net.HttpStatusCode.OK }));

        }
        catch (System.Exception)
        {

            _logger.LogError($"[{DateTime.Now}] -- Error deleting blog");
